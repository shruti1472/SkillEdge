    ...
    <div className='min-h-screen w-full px-4 py-9 bg-gray-50'>
...

     {
      userData?.enrolledCourses?.length === 0 ? (
        <p className='text-gray-500 text-center w-full'>You haven't enrolled in any course yet.
        </p>
          ...
          (
              <div key={index} className='bg-white rounded-2xl shadow-md overflow-hidden border'>
                ...
                <div className='p-4'>
                  ...
                  <h1 className='px-[10px] border-2 bg-black border-black text-white rounded-[10px] text-[15px] font-light flex items-center justify-center gap-2 cursor-pointer mt-[10px] hover:bg-gray-600' onClick={()=>navigate(`/viewlecture/${course._id}`)}>Watch Now</h1>
                </div>
              </div>
          ))}
        </div>
