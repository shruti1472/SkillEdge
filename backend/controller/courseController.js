...
export const createCourse = async (req, res) => {
    try {
        ...
        // Use the correct userId from auth middleware
...
export const getPublishedCourses = async (req,res) => {
    try{
        ...
        return res.status(200).json(courses)
         ...
         return res.status(500).json({message:`failed to find the isPublished courses ${error}`})
    }
}
...
export const getCourseLecture = async(req,res) =>{
    try{
         ...
         return res.status(200).json(course)
          ...
          return res.status(500).json({message:`failed to to get course lecture ${error}`})
    }
}
...
export const removeLecture = async (req,res) => {
    ...
    return res.status(200).json({message:"Lecture is removed"})
        ...
        return res.status(500).json({message:`failed to to remove  the lecture ${error}`})
